...
- (void)makeToast:(NSString *)message duration:(CGFloat)interval {
    [self makeToast:message duration:interval position:kDefaultPosition];
}
...
- (UIView *)makeViewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (title != nil) {
        ...
        CGRect expectedSizeTitle = [title boundingRectWithSize:CGSizeMake(titleLabel.frame.size.width,maxSizeTitle.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:17.0]} context:nil];
//        CGSize expectedSizeTitle = [title sizeWithFont:titleLabel.font constrainedToSize:maxSizeTitle lineBreakMode:titleLabel.lineBreakMode]; 
        [titleLabel setFrame:CGRectMake(0.0, 0.0, expectedSizeTitle.size.width, expectedSizeTitle.size.height)];
    }
    ...
    if (message != nil) {
        ...
        CGRect expectedSizeMessage = [message boundingRectWithSize:CGSizeMake(titleLabel.frame.size.width,maxSizeMessage.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:17.0]} context:nil];
//        CGSize expectedSizeMessage = [message sizeWithFont:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];
        [messageLabel setFrame:CGRectMake(0.0, 0.0, expectedSizeMessage.size.width, expectedSizeMessage.size.height)];
    }
